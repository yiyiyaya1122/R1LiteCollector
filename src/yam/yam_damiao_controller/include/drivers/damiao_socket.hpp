...
#include "communication/SocketCAN.hpp" // now SocketCAN-backed SerialPort
#include <cmath>
#include <utility>
#include <vector>
...
inline Limit_param limit_param[Num_Of_Motor]=
    ...
    private:
        ...
        uint8_t error_code_ = 0; // 存储错误代码
        bool has_error_ = false; // 错误状态标志
    ...
    public:
        ...
        // 新增错误处理方法
        void set_error_flag(uint8_t error_code) {
        ...
        std::string get_error_description() {
            switch (error_code_) {
                case 0x08: return "Over voltage";
                case 0x09: return "Under voltage";
                case 0x0A: return "Over current";
                case 0x0B: return "MOS over temperature";
                case 0x0C: return "Motor coil over temperature";
                case 0x0D: return "Communication lost";
                case 0x0E: return "Overload";
                default:   return "Unknown error " + std::to_string(error_code_);
            }
        }
 ...
 */
    ...
    public:
        ...
        */
        Motor_Control(SerialPort::SharedPtr serial = nullptr): serial_(std::move(serial))
        {
            if (serial_ == nullptr) {
                // default to can0
                serial_ = std::make_shared<SerialPort>("can0", 0);
            }
        }
        ...
        */
        void enable(const Motor& motor)
        {
            ...
            usleep(100000); // 100ms origin
            this->receive();
        }

        /*
        * @brief clear the motor error
        * @param motor 电机对象
        */
        void clear_error(const Motor& motor)
        {
            control_cmd(motor.GetSlaveId(), 0xFB);
            this->receive();
        }
        ...
        void refresh_motor_status(const Motor& motor)
        {
            ...
            // send as raw CAN
            serial_->send_can(id, data_buf.data(), 8, false);
            this->receive();
        }
        /*
        ...
        */
        ...
        void receive()
        {
            ...
            ssize_t n = serial_->recv_raw(frame);
            if (n <= 0) {
                return; // timeout or error
            }

            // convert struct can_frame -> CAN_Receive_Frame (your existing format)
            std::memset(&receive_data, 0, sizeof(receive_data));
            receive_data.FrameHeader = 0xAA; // keep same head that original code searched for
            receive_data.CMD = 0x11; // indicate receive success
            receive_data.canDataLen = frame.can_dlc & 0x3F;
            receive_data.canIde = (frame.can_id & CAN_EFF_FLAG) ? 1 : 0;
            receive_data.canRtr = (frame.can_id & CAN_RTR_FLAG) ? 1 : 0;
            // mask id to remove flags
            uint32_t raw_id = frame.can_id & (receive_data.canIde ? CAN_EFF_MASK : CAN_SFF_MASK);
            receive_data.canId = raw_id;
            std::memcpy(receive_data.canData, frame.data, frame.can_dlc);
            receive_data.frameEnd = 0x55;

            // now reuse your existing parsing logic
            if(receive_data.CMD == 0x11 && receive_data.frameEnd == 0x55) // receive success
            {
                ...
                // 解析错误代码
                // high 4bit for error code
                uint8_t error_code = (receive_data.canData[0] >> 4) & 0x0F;
                Motor_id motor_id = receive_data.canId;
                
                // 0 for disable 1 for enable, other for error code
                if (error_code != 0 && error_code != 1) {
                    // 查找对应的电机对象
                    auto it = motors.find(motor_id);
                    if (it != motors.end()) {
                        it->second->set_error_flag(error_code);
                    }
                    else {
                        // 备用查找逻辑（当masterID为0x00时）
                        uint32_t slaveID = receive_data.canData[0] & 0x0f;
                        auto it2 = motors.find(slaveID);
                        if (it2 != motors.end()) {
                            it2->second->set_error_flag(error_code);
                        }
                    }                
                }
                return;
        ...
        /**
        * @brief 检查电机错误状态
        * @param motor 要检查的电机对象
        * @return 错误代码 (0表示无错误)
        */
        uint8_t check_error(const Motor& motor) const {
            Motor_id id = motor.GetSlaveId();
            auto it = motors.find(id);
            if (it != motors.end() && it->second->has_error()) {
                return it->second->get_error_code();
            }
            return 0; // 无错误
        }
        ...
        void receive_param()
        {
            ...
            // convert to receive_data for reuse of parsing logic
            std::memset(&receive_data, 0, sizeof(receive_data));
            receive_data.FrameHeader = 0xAA;
            receive_data.CMD = 0x11;
            receive_data.canDataLen = frame.can_dlc & 0x3F;
            receive_data.canIde = (frame.can_id & CAN_EFF_FLAG) ? 1 : 0;
            receive_data.canRtr = (frame.can_id & CAN_RTR_FLAG) ? 1 : 0;
            receive_data.canId = frame.can_id & (receive_data.canIde ? CAN_EFF_MASK : CAN_SFF_MASK);
            std::memcpy(receive_data.canData, frame.data, frame.can_dlc);
            receive_data.frameEnd = 0x55;
        ...
        float read_motor_param(Motor &DM_Motor,uint8_t RID)
        {
            ...
            serial_->send_can(0x7FF, data_buf.data(), 8, false);
    ...
    private:
        ...
        SerialPort::SharedPtr serial_;  // now SocketCAN-backed serial port adapter
        can_send_frame send_data; // kept for compatibility if you want to build frames elsewhere
