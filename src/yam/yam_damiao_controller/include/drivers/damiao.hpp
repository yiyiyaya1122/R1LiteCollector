...
#include "communication/SerialPort.hpp"
#include <cmath>
#include <utility>
#include <vector>
 ...
 */
    ...
    public:
        ...
        */
        ...
        void control_pos_force(Motor &DM_Motor,float pos, uint16_t vel, uint16_t i)
        {
            ...
            // std::cout << "已发送数据：" << data_buf.size() << " 字节" << std::endl;
    ...
    private:
        void control_cmd(Motor_id id , uint8_t cmd)
        {
            ...
            uint8_t* raw_ptr = reinterpret_cast<uint8_t*>(&send_data);
            // for (size_t i = 0; i < sizeof(can_send_frame); ++i) {
            //     printf("%02X ", raw_ptr[i]); // 或使用 std::cout 版本
            // }
            // printf("\n");
